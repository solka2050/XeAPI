...
router.get("/", (req, res) => {
 ...
 // console.log(req.params);
  var sqlqueryupdates="SET @varUserId=?;CALL ProfileViewer(@varUserId)"
  
                      mysqlConnection.query(
                        sqlqueryupdates,[req.query.varUserId],
                        (err, results, fields) => {
                          if (!err) {
                            console.clear();
                             console.log(results);
...
router.post("/", (req, res) => {
  var sql =
    "SELECT id, Code, Name, Amount, SalePrice, BuyPrice,(SELECT Name FROM DealerData where idDealerData=UserId) As Dealer, OrderType, Status,UserId, Visibility, DATE_FORMAT(DateAdded, '%d-%m-%Y') AS Date FROM CurrencyData where Visibility='Visible' AND Status='Open';";
