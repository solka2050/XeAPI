...
router.get("/", (req, res) => {
  ...
  EntryType, Description, LocationName,VehRegNo FROM dhulltra_benz.Expenses where VehRegNo=" +vechino +" && WorkDate>=" +fromdate +" && WorkDate<=" +todate +" ORDER BY DATE(WorkDate);";
...
select idExpenses, DATE_FORMAT(WorkDate, '%d-%m-%Y') AS WorkDate,FORMAT(Rent,0,'en_IN') AS Rent,\
  FORMAT(DelhiEntry,0,'en_IN') AS DelhiEntry,FORMAT(Diesel,0,'en_IN') AS Diesel,FORMAT(Repairing,0,'en_IN') AS Repairing, RepairingId,FORMAT(Food,0,'en_IN') AS Food,\
  FORMAT(Kaanta,0,'en_IN') AS Kaanta,FORMAT(TollTax,0,'en_IN') AS TollTax,FORMAT(Other,0,'en_IN') AS Other,FORMAT(Total,0,'en_IN') AS Total,FORMAT(Balance,0,'en_IN') AS Balance,\
  EntryType, Description, LocationName,VehRegNo FROM dhulltra_benz.Expenses where VehRegNo=" +vechino +" && WorkDate>=" +fromdate +" && WorkDate<=" +todate +" &&  Repairing>0 && EntryType <>'Monthly Expenses' \
...
FORMAT(sum(TollTax),0,'en_IN') AS TollTax,FORMAT(sum(Other),0,'en_IN') AS Other,FORMAT(sum(Total),0,'en_IN') AS Total,FORMAT(sum(Balance),0,'en_IN') AS Balance FROM dhulltra_benz.Expenses
 where VehRegNo='HR-56-B-3692' && WorkDate>'2020-05-01 00:00:00' && WorkDate<'2020-05-14 23:59:59';*/
router.get("/:idExpenses", (req, res) => {
  mysqlConnection.query(
    "SELECT * FROM dhulltra_benz.Expenses where idExpenses=?;",
