...
router.get("/", (req, res) => {
    mysqlConnection.query("SELECT max(RepairId)+1 AS NewRepairId FROM dhulltra_benz.Det_Repair_Entry;", (err, results) => {
        if (err) {
...
mysqlConnection.query('INSERT INTO `dhulltra_benz`.`Det_Repair_Entry`(`RepairId`,`LineNo`,`Description`,`Amount`) VALUES ?', [values], function(err,result) {
    if (!err) {
    ...
    let user = req.body;
    console.log(user);
    let query = "INSERT INTO `dhulltra_benz`.`Det_Repair_Entry`(`RepairId`,`LineNo`,`Description`,`Amount`) VALUES ?";
    ...
    CALL dhulltra_benz.sp_new_det_repair_entry(@varRepairId,@varLineNo,@varDescription,@varAmount);";
