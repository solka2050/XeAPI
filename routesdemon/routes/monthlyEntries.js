...
router.get("/", (req, res) => {
  ...
  var sqlstring="select count(*) AS TotalTripCount FROM dhulltra_benz.Expenses where VehRegNo=" +vechino +" && WorkDate>=" +fromdate +" && WorkDate<=" +todate +" && LocationName not in('Khali','M-Khali','M-Khali-','Repair');";
...
router.post("/", (req, res) => {
  ...
  var sql =
    ...
    CALL dhulltra_benz.NewMonthlyEntry(@varWorkDate,@VarAmount,@VarAmount2,@VarAmount3,@VarTotalRounds,@varVehRegNo);";
  //  console.log(sql);
  mysqlConnection.query(
    sql,
    [
