...
router.get("/", (req, res) => {
    mysqlConnection.query("SELECT * FROM dhulltra_benz.Drivers;", (err, results) => {
        if (err) {
...
router.post("/", (req, res) => {
    let user = req.query;
    mysqlConnection.query("UPDATE `dhulltra_benz`.`Drivers` SET `Status` =" + user.varStatus + " WHERE `idDrivers` =" + user.id + ";", (err, results) => {
        if (err) {
